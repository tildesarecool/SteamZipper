...
function Run-Test {
    ...
    try {
        ...
        $output = & pwsh -Command $Command -ErrorAction Stop 2>&1
        ...
        if ($LASTEXITCODE -ne 0 -or ($outputStr -match "Parameter set cannot be resolved")) {
...
function Verify-Results {
    ...
    if ($Expected.ContainsKey("ZipCount")) {
        ...
        if ($null -eq $Expected.ZipCount) {
            $details += "Zip count check skipped (expected null)"
...
$results += Run-Test -TestName "CreateAnswerFile" `
    ...
    -Expected @{ ZipCount = 14; AnswerFile = $true }
